...
class ProductionSimulatorFull:
    ...
    def __init__(self, 
                 ...
                 news_database_path: Optional[str] = None):
        """
        ...
        self.news_database_path = news_database_path
        
        # Load news database if provided
        self.news_database = None
        if news_database_path and Path(news_database_path).exists():
            import json
            with open(news_database_path, 'r') as f:
                self.news_database = json.load(f)
            logger.info(f"Loaded news database: {len(self.news_database)} tickers")
