"""
Enhanced simulator using robust trend detection with moderate strategy.
Adjusts threshold based on trend strength instead of multiplying signal.
"""
...
class RobustEnhancedSimulator:
    """Trading simulator using robust trend detection with moderate strategy."""
    ...
    def calculate_signal_score_robust(self, signals, data, base_threshold=0.5):
        """
        Calculate signal score with robust trend filtering (MODERATE STRATEGY).
        
        Instead of multiplying signal strength, adjusts the threshold:
        - Strong consolidation: threshold = 0.5 (normal)
        - Weak trend: threshold = 0.4 (easier to trigger)
        - Moderate trend: threshold = 0.55 (harder to trigger)
        - Strong trend: threshold = 0.65 (much harder to trigger)
        """
        if not signals:
            return 0, None, 0.5, "No signals"
        ...
        try:
            ...
            # Fallback if trend detection fails
            consensus = 'unknown'
            confidence = 0.5
        ...
        if consensus == 'consolidation':
            ...
            trend_note = "Consolidation"
        elif consensus == 'downtrend':
            if confidence >= 0.8:
                adjusted_threshold = base_threshold + 0.15  # 0.65 (hard to trigger)
                trend_note = f"Strong downtrend - high threshold"
            elif confidence >= 0.5:
                adjusted_threshold = base_threshold + 0.05  # 0.55 (slightly harder)
                trend_note = f"Moderate downtrend - higher threshold"
            else:
                adjusted_threshold = base_threshold - 0.05  # 0.45 (easier)
                trend_note = f"Weak downtrend - normal threshold"
        elif consensus == 'uptrend':
            if confidence >= 0.8:
                adjusted_threshold = base_threshold - 0.1  # 0.4 (easier to trigger)
                trend_note = f"Strong uptrend - low threshold"
            else:
                adjusted_threshold = base_threshold  # 0.5 (normal)
                trend_note = f"Weak uptrend - normal threshold"
        else:
            adjusted_threshold = base_threshold
            trend_note = "Unknown trend"
        
        return avg_strength, direction, adjusted_threshold, trend_note
    
    def simulate_ticker(self, ticker, start_date, end_date, base_threshold=0.5, use_robust=True):
        """Simulate trading with robust trend detection."""
        ...
        for i in range(window_size, len(data)):
            ...
            if use_robust:
                signal_score, signal_direction, threshold, trend_note = self.calculate_signal_score_robust(
    ...
    def run_backtest(self, tickers, start_date, end_date, base_threshold=0.5, use_robust=True):
        """Run backtest with robust trend detection."""
        mode = "WITH ROBUST TREND (Moderate Strategy)" if use_robust else "WITHOUT TREND FILTER"
        ...
        for ticker in tickers:
            result = self.simulate_ticker(ticker, start_date, end_date, base_threshold, use_robust)
            if result:
...
if __name__ == '__main__':
    print("\n\n" + "=" * 70)
    print("COMPARISON: Robust Trend Detection (Moderate) vs Simple")
    print("=" * 70)
    ...
    # Without robust filter
    print("\n\n" + "🔴" * 35)
    print("\nWithout Trend Filter (Original):")
    simulator_no_filter = RobustEnhancedSimulator()
    results_no_filter = simulator_no_filter.run_backtest(
        test_tickers, start_date, end_date, use_robust=False
    )
    
    # With robust trend filter
    print("\n\n" + "🟢" * 35)
    print("\nWith Robust Trend Detection (Moderate Strategy):")
    simulator_with_robust = RobustEnhancedSimulator()
    results_with_robust = simulator_with_robust.run_backtest(
        test_tickers, start_date, end_date, use_robust=True
    )
    ...
    print(f"\n{'Ticker':<10} {'Without Filter':<30} {'With Robust Trend':<30}")
    print("-" * 70)
    
    for i, ticker in enumerate(test_tickers):
        if i < len(results_no_filter) and i < len(results_with_robust):
            r_no = results_no_filter[i]
            r_robust = results_with_robust[i]
            
            print(f"{ticker:<10} Return: {r_no['total_return_pct']:+6.1f}%, Win: {r_no['win_rate_pct']:5.0f}%   |   Return: {r_robust['total_return_pct']:+6.1f}%, Win: {r_robust['win_rate_pct']:5.0f}%")
